...
import { GroupInfoData, GroupInfoResponse, Particapant, groupInfo } from "../functions/groupInfo";
import { dayToLetter, monthToLetter } from "../functions/dateConversion";
import { declineParticapant } from "../functions/declineParticapant";
import { acceptParticapant } from "../functions/acceptParticapant";
import { removeParticapant } from "../functions/removeParticapant";
...
export default function GroupInfo({ groupId, othersCanAdd, close }: { groupId: string, othersCanAdd: boolean, close: () => any }) {
    ...
    async function particapantAccept(id: string, name: string) {
        if (!data2) {
            setError({ message: "Failed", type: "alert", show: true});
            return
        }
        const datares = await acceptParticapant(groupId, token!, id);
        if (datares.error || !datares.message) {
            setError({ message: datares.error ? datares.error : "Failed", type: "alert", show: true});
            return;
        }
        let newParticapants = [];
        ...
        setError({message: "Success", type: "success", show: true});
    }
    ...
    async function removeParticapantFunc(id: string) {
        ...
        if (removeParticapantData.error || !removeParticapantData.message) {
            setError({message: removeParticapantData.error ? removeParticapantData.error : "Failed", type: "alert", show: true});
            return;
        }
        if (!data2) return
        let newParticapants = [];
