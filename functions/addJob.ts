...
export async function addJob(token: string, job: JobToAdd): Promise<AddJobResponse> {
    if (job.day === 0 || job.month === 0 || job.year === 0) return {error: "Invalid Date"};
    if (job.hour === 0 || job.minute === 0) return {error: "Invalid Time"};
    if (job.jobTitle.length === 0) return {error: "You Must add a job title."};
    if (job.group.length === 0) return {error: "Group not selected."};
    if (job.positions <= 0) return {error: "Positions must be greater than '0'"};
